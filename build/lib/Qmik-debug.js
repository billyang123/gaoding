...
(function() {
    ...
    con = win.console, UA = nav.appVersion || nav.userAgent, loc = win.location;
    ...
    function merge() {
        ...
        var args = arguments, array = args[0], isA = isArray(array);
    ...
    function loadResource(type, url, success, error) {
        ...
        tagName = isCss ? "link" : isScript ? "script" : "iframe", node = doc.createElement(tagName), qnode = Q(node).attr({
            _src: url,
            async: "async"
        });
        ...
        if (isCss) node.href = url; else node.src = url;
        Q("head").append(node);
        return node;
    }
    /**
	...
	*/
    ...
    Q.extend({
        ...
        inArray: function(value, array) {
            if (Q.likeArray(array)) for (var i = 0; i < array.length; i++) if (array[i] === value) return i;
            return -1;
        },
        ...
        map: function(array, callback) {
            var r = [], i = 0;
            for (;array && i < array.length; i++) isNull(array[i]) || r.push(callback(i, array[i]));
    ...
    each([ Q.url, Q.now ], function(i, val) {
        val.toString = val;
    });
    //不对外部开放,不保持此对象api不变动,
    Q._in = {
...
(function(Q) {
    ...
    /* 节点查询编译 */
    var rNode = /^\s*(<.+>.*<\/.+>)+|(<.+\/\s*>)+\s*$/, addUints = "height width top right bottom left".split(" ");
    /** Qmik查询 */
    function Query(selector, context) {
        ...
        if (isString(selector)) {
            ...
            r = likeArray(selector) ? selector : [ selector ];
        }
    ...
    function hasClass(dom, className) {
        ...
        for (;i < cs.length; i++) if (cs[i] == className) return !0;
        return !1;
    }
    ...
    fn.extend({
        ...
        html: function(v) {
            var me = this;
            if (arguments.length < 1) return attr(me, "innerHTML"); else {
        ...
        inViewport: function() {
            var qdom = this, offset = qdom.offset(), bool = false;
            if (offset) {
                var elTop = offset.top, elLeft = offset.left, elDown = elTop + qdom.height(), elRight = elLeft + qdom.width(), minX = win.pageXOffset, maxX = getMaxX(), minY = win.pageYOffset, maxY = getMaxY();
                minY = minY < 0 ? 0 : minY;
        ...
        children: function(selector) {
            ...
            me.each(function(i, dom) {
                ...
                var childs = dom.children || dom.childNodes, j = 0, tdom;
...
(function(Q) {
    ...
    ek = "QEvents", liveFuns = {};
    var isNull = Q.isNull, isFun = Q.isFun, each = Q.each, isPlainObject = Q.isPlainObject, _delete = _in._delete;
    /** 设置节点的加载成功方法 */
    ...
    function Erm(dom, name, fun) {
        ...
        if (fun) {
            for (;i >= 0; i--) h[i].fun == fun && h.splice(i, 1);
        } else {
    ...
    function handle(_event) {
        ...
        each(events[_event.type], function(i, v) {
            Q.execCatch(function() {
                ...
                param = v.param || [];
                if (isFun(fun)) {
                    retVal = fun.apply(m, [ _event ].concat(param));
    ...
    function mapEvent(name, fun, dealFun) {
        var ents = {};
        if (isPlainObject(name)) ents = name; else ents[name] = fun;
    ...
    fn.extend({
        ...
        live: function(name, callback) {
            ...
            mapEvent(name, callback, function(key, callback) {
                var fun = me.__lives[getLiveName(key, callback)] = function(e) {
                    var target = e.target, qtar = Q(target), sel = Q.isString(me.selector) ? Q(me.selector, me.context) : me;
        ...
        die: function(name, callback) {
            var me = this, fun = me.__lives[getLiveName(name, callback)];
...
(function(Q) {
    ...
    _delete = Q._in._delete, regUrl = /[\w\d_$-]+\s*=\s*\?/, jsonp = 1, prefex = "qjsonp", //
    ac = {
        type: "GET",
        async: !0,
        dataType: "text",
...
(function(Q) {
    ...
    function execParallelTasks(tasks, callback) {
        var length = tasks.length, params = new Array(length);
        var pageSize = parseInt((length - 1) / dealTasks) + 1;
        //每组要处理的长度
        var groups = new Array(dealTasks < length ? dealTasks : length);
        //几组
...
(function(Q) {
    var isFun = Q.isFun, win = Q.global, NULL = null;
    var config = {
        alias: {},
        //别名系统
        vars: {},
    ...
    function Module(url, dependencies, factory) {
        Q.extend(this, {
            ...
            state: 3,
            // is ready ,default false, 1=ok,2=准备中,3=寻找模块
            type: Q.inArray(url, config.preload) >= 0 ? 2 : 1,
            //2:预加载的类型,1:普通类型
            exports: {}
        });
    }
    ...
    function parseDepents(code) {
        ...
        var require = params.substring(0, idx > 0 ? idx : params.length), pattern = new RegExp(require + "s*[(]s*[\"']([^\"')]+)[\"']s*[)]", "g");
        if (require) match = Q.map(code.match(pattern), function(i, v) {
    ...
    Q.extend(sun, {
        ...
        define: function(uid, dependencies, factory) {
            ...
            define("lib/Qmik-debug.all", [], dependencies);
            currentScript = NULL;
        },
    ...
    Q.sun = sun;
    Q.define = sun.define;
    win.define = win.define || Q.define;
    //如果外面没有引入其它cmd框架,设置全局变量define
    Q.use = sun.use;
})(Qmik);
 ...
 */
(function(Q) {
    var win = Q.global, isNull = Q.isNull, isPlainObject = Q.isPlainObject, execCatch = Q.execCatch, extend = Q.extend, each = Q.each, delay = Q.delay, con = console;
    var ctrls = {}, //控制器存储
    scopes = {}, g_config = {
        section: 24
    }, keywords = "scopes context parent get set on off once app watch apply $";
    //关键词,用户不能定义到scope上的变量名
    var nameParentScope = "parent", namespace = "qmik-mvc-space", namespaceScope = "qmik-mvc-space-scope", fieldWatchs = "__watchs", nameRoot = "html", nameContext = "context", nameInput = "__input", nameMap = "__map", execInterval = 10;
    //scroll触发间隔
    /********* 当节点在显示视口时触发 start *******/
    var g_viewports = {};
    var prevTime = Q.now();
    function handle() {
        var curTime = Q.now();
        if (curTime - prevTime < execInterval) {
            //触发频率
        ...
        each(map, function(key, map) {
            var node = map.scope ? map.scope.context : map.context, qdom = Q(node);
            if (qdom.inViewport()) {
    ...
    function watch(e, scope, emit) {
        var target = e.target, name = emit ? e.name : target.name || "", scope = getCtrlNode(target)[namespaceScope] || scope;
    ...
    extend(App.prototype, {
        __init: function(fun) {
            var me = this, scope = new Scope(), root = Q(nameRoot)[0];
            ...
            function remove(e) {
                var target = e.target, name = target.name, isInputDom = isInput(target), space = getSpace(target);
                if (space) {
            ...
            Q(win).on({
                ...
                DOMNodeInserted: function(e) {
                    //节点增加
                    var target = e.target, space = getSpace(target), scope = space.scope;
                    if (space) {
    ...
    function Scope(context, rootScope) {
        ...
        me[fieldWatchs] = {};
        //监听器集合
        me[nameContext] = context = context || Q(nameRoot)[0];
        //上文dom节点
        me.scopes = scopes;
        me.__name = Q(context).attr("q-ctrl") || "root";
        //控制器名
        me[nameMap] = {};
        //变量映射节点集合
        me.__cmd = {};
    ...
    function uniqueArray(list1, list2) {
        ...
        var list = list1.concat(list2 || []).sort(), result = [];
    ...
    function getInputValue(node) {
        var name = node.name, type = node.type, vals = [];
        if (type == "radio") {
    ...
    function fieldValue(object, names, val) {
        var ns = Q.isArray(names) ? names : split(names), field = ns[0];
        if (ns.length < 2) {
            if (!isNull(val)) {
        ...
        switch (node.nodeType) {
          case 1:
            ...
            node.tagName != "SCRIPT" && each(node.attributes, function(i, attr) {
                ...
                } else if ("q-for" == attrName) {
                    //for
                    var vs = value.replace(/(\s){2,}/g, " ").split(" "), template = space.html = space.html || node.innerHTML.replace(REG_SCRIPT, "&lt;script"), htmls = [], list = getVarValue(scope, vs[2]) || [];
                    if (vs.length == 3 && vs[1] == "in") {
                        each(list, function(i, item) {
                            var html = template.replace(REG_VAR_NAME, function(varName) {
                                var reg = new RegExp("^" + vs[0] + "."), name = getVarName(varName).replace(reg, ""), val = fieldValue(item, name);
                                val = isNull(val) ? "" : val + "";
                ...
                } else if (/^q-on/.test(attrName)) {
                    //事件绑定
                    var name = attrName.replace(/^q-on/, ""), funName = value.replace(/\(.*\)$/, ""), context = scope[nameContext];
                    if (!space.event[name]) {
                        node[namespace] = space;
                ...
                } else if (REG_VAR_NAME.test(value)) {
                    //变量
